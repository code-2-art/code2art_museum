...
- 保持 Astro 静态主站与 `docs/` 发布方式，3D 版本暂时冻结。
- 公开馆藏继续随站点构建，提供可工作的 Agent 检索、档案筛选与展品详情。
- 使用 Supabase Auth、Postgres、Storage 和 RLS 承载用户投稿、私有附件与策展审核。
- 投稿遵循“用户提交 → 策展审核 → 确认来源与授权 → 发布为公开馆藏”的流程，不自动公开。
- 在接入生成式模型前，Museum Agent 先使用确定性的结构化检索返回带出处的答案；模型密钥只允许存在于服务端环境。
...
- 第一阶段：静态交互原型。已完成。
- 第二阶段：结构化检索。已完成首版，本地馆藏可按意图检索并返回关联证据。
